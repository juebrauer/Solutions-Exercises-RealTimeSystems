...
int main()
{
  ...
  // 1. generate an EDF scheduler
  int quantum = 10;
  scheduler_edf s( quantum );


  // 2. ask for number of tasks, execution times and periods
  ...
  // 3. show results of schedulability test
  s.schedulability_test(info_all_periodic_tasks);


  // 4. compute kgV of period times
  ...
  while ((deadline_violation == false) && (simulation_time < simulation_end_time))
  {
    ...
    // 5.3 restart periodic tasks if needed
    for (int i = 0; i < nr_tasks; i++)
    {
      // get the i-th task
      task* t = info_all_periodic_tasks[i];
    ...
    // 5.4 prepare a string that contains the information which tasks waited
